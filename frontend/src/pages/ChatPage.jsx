...
export default function ChatPage() {
          ...
          for (let line of lines) {
            if (line.startsWith('data: ')) {
              ...
              try {
                ...
                console.error("Parse JSON error during stream line:", dataStr, e)
              }
            }
          }
        }
  ...
  const getSourceBadgeColor = (type) => {
    switch(type) {
      case 'posts': return 'bg-blue-900 border-blue-500 text-blue-300'
      case 'graph_facts': return 'bg-green-900 border-green-500 text-green-300'
      case 'topic_summaries': return 'bg-purple-900 border-purple-500 text-purple-300'
      default: return 'bg-gray-800 border-gray-500 text-gray-300'
    }
  }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] max-w-4xl mx-auto bg-gray-800 rounded-xl border border-gray-700 shadow-2xl overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-700 bg-gray-900 flex justify-between items-center z-10 shadow-sm relative">
        <h2 className="text-xl font-bold font-mono tracking-tight text-gray-200">RAG Chat Assistant</h2>
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-gray-500 gap-4 mb-20 fade-in">
             <div className="bg-gray-900 p-6 rounded-2xl border border-gray-700 shadow-xl max-w-md text-center">
                 <h3 className="text-white font-bold mb-2">Ask the Data Anything!</h3>
                 <p className="text-sm">Query the dataset securely using multi-source RAG embeddings across posts, topic clusters, and community graphs.</p>
             </div>
          </div>
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
            <div className={`max-w-[85%] rounded-2xl px-5 py-4 ${
              ...
              {/* Language Alert */}
              {m.meta && m.meta.language !== 'en' && m.meta.language !== 'unknown' && (
              ...
              {/* Message Content */}
              <div className="whitespace-pre-wrap text-[15px] leading-relaxed font-sans" dangerouslySetInnerHTML={{
                // Quick hack for bold markdown rendering easily in simple chat
                __html: m.content ? m.content.replace(/\\*\\*(.*?)\\*\\*/g, '<strong>$1</strong>') : (m.role === 'assistant' ? '<span class="animate-pulse">Thinking...</span>' : '')
              }}></div>
                  ...
                  <details className="text-sm marker:text-gray-500">
                    ...
                    <div className="mt-4 space-y-3 pl-2 border-l-2 border-gray-800">
                      {m.meta.sources.map((src, sIdx) => (
                        <div key={sIdx} className="bg-gray-800 rounded p-3 text-xs text-gray-400">
                           <div className="flex justify-between items-center mb-2">
                             <span className={`px-2 py-[2px] rounded uppercase font-bold border shrink-0 text-[10px] ${getSourceBadgeColor(src.source_type)}`}>
                               {src.source_type.replace('_', ' ')}
                             </span>
                             <span className="text-[10px] text-gray-600 font-mono">Similarity: {(src.score * 100).toFixed(0)}%</span>
                           </div>
                           <p className="italic line-clamp-3 leading-relaxed mt-1">"{src.text}"</p>
                        </div>
                      ))}
                    </div>
                  </details>
              ...
              {m.suggestions && m.suggestions.length > 0 && (
                <div className="mt-4 flex flex-col gap-2 pt-4 border-t border-gray-800">
                  <span className="text-xs text-gray-500 font-bold uppercase">Suggested Follow-ups:</span>
      ...
      {/* Input */}
      <form onSubmit={(e) => handleSubmit(e, null)} className="p-4 bg-gray-900 border-t border-gray-700 flex gap-4">
        <input
          type="text"
          value={query}
