...
export default function NetworkPage({ spamThreshold }) {
  ...
  const handleAuthorPosts = async (authorName) => {
    try {
      const res = await axios.get(
        `http://127.0.0.1:8000/posts?author=${authorName}`,
      );
  ...
  const handleRemoveNode = async () => {
    ...
    try {
      const res = await axios.post(
        "http://127.0.0.1:8000/network/remove-node",
        {
  ...
  const nodeDegree = sid
    ? (allMetrics[sid]?.degree ?? selectedNode?.degree ?? "—")
    : null;
  const nodePR = sid
    ? (allMetrics[sid]?.pagerank?.toFixed(6) ??
      selectedNode?.pagerank?.toFixed(6) ??
      "—")
    : null;
  const nodeBetw = sid
    ? (allMetrics[sid]?.betweenness?.toFixed(2) ??
      selectedNode?.betweenness?.toFixed(2) ??
      "—")
    : null;
      ...
      <div className="flex gap-4 flex-1 min-h-0" style={{ overflow: "hidden" }}>
        ...
        <div className="flex flex-col gap-4 w-64 shrink-0 min-h-0 overflow-y-auto">
          ...
          <div className="bg-gray-800 rounded border border-gray-700 p-4 shrink-0">
            <h3 className="font-bold text-gray-200 mb-3 text-sm uppercase tracking-wider">
              Node Info
            </h3>
            {selectedNode ? (
              <div className="space-y-2 text-sm">
                <div className="bg-gray-900 rounded p-2 border border-gray-700">
                  ...
                  <p className="text-white font-medium break-all mt-0.5">
                    {selectedNode.id}
                  </p>
                </div>
            ...
            <ul className="space-y-2">
              {data?.metrics?.labels ? (
                Object.entries(data.metrics.labels).map(([id, info]) => (
                  <li
                    key={id}
                    className="flex flex-col gap-1 text-sm bg-gray-900 p-2 rounded border border-gray-700"
                  >
                    ...
                    <span className="text-gray-500 text-xs ml-5">
                      Group {id}
                    </span>
                  </li>
                ))
